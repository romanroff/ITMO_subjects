...
def generate_dataset(
    data, seq_len, pre_len, time_len=None, split_ratio=0.8, normalize=True, fourier=True, threshold=0.001
):
    """
    ...
    for i in range(len(train_data) - seq_len - pre_len):
        if fourier:
            train_X.append(np.array(apply_fourier_denoising(train_data[i: i + seq_len], threshold)))
        else:
            train_X.append(np.array(train_data[i: i + seq_len]))
        train_Y.append(np.array(train_data[i + seq_len: i + seq_len + pre_len]))
    for i in range(len(test_data) - seq_len - pre_len):
        if fourier:
            test_X.append(np.array(apply_fourier_denoising(test_data[i: i + seq_len], threshold)))
        else:
...
def generate_torch_datasets(
    data, seq_len, pre_len, time_len=None, split_ratio=0.8, normalize=True, fourier=False, threshold=0.001
):
